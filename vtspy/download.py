...
class DownloadFermiData:
    ...
    Args:
        ...
        verbosity (int)
    """
    def __init__(self, file = "config.yaml", verbosity = 1):
        
        self.logging = logger(verbosity=verbosity)
        ...
        if self.loc[0] == None or self.loc[1] == None:
            self.logging.error("[Error] Coordinates (e.g., RA & DEC) is not specified.")
            return
        
        self.tmin = self.config['selection']['tmin']
        self.tmax = self.config['selection']['tmax']    
        if self.tmin == None or self.tmax == None:
            self.logging.error("[Error] Time range is not specfied.")
            return
    ...
    def __query__(self):
        ...
        self.logging.info("Query parameters:")
        for k,v in parameters.items():
            self.logging.info("%30s = %s" %(k,v))
        ...
        self.logging.debug("Answer from the LAT data server:")
        ...
        for t in text:
            if "occurs after data end MET" in t:
                ...
                self.logging.error("[Error] The current Fermi Data Server does not have data upto the entered 'tmax'.")
                self.logging.error("[Error] 'tmax' value in the config file is changed to the maximum value.")
                self.logging.error("[Error] config['selection']['tmax'] = ", maxTime)
                self.logging.error("[Error] Please try again.")
        ...
        for t in text: self.logging.debug(t)
        ...
        self.logging.info("The estimated time is about "+str(int(estimatedTimeForTheQuery))+" seconds.")
        while(time.time() <= startTime+timeout):
            remainedTime = int(int(estimatedTimeForTheQuery) - (time.time()-startTime))
            if remainedTime<0 and not(overTime):
                overTime=True
                self.logging.info("The Fermi data is still not ready. Wait for another " + str(int(estimatedTimeForTheQuery)) + " seconds.")
            try:
        ...
        try:
            ...
            self.logging.error("[Error] The files (SC and EV files) are not ready to be downloaded. Check the link and then use 'DownloadFermiData.manualDownload()' when the data is ready.")
            return
    ...
    def __download__(self):
        ...
        for lk in links:
            self.logging.info("Downloading... "+lk)
            fileName = lk[-9:-5]
        ...
        self.logging.info("Downloading the Fermi-LAT data has been completed.")
        os.system("rm ./fermi/fermi_dwn_link.npy")
        config.updateConfig(self.config, self.file)

    def manualDownload(self, address=None):
        """
